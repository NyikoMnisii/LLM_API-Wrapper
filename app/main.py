...
async def lifespan(app: FastAPI):
    ...
    tool_executor = ToolExecutor(specs=[build_weather_tool_spec(app.state.weather_service)])
    gemini_service = GeminiService(
        client=app.state.gemini_client,
        tool_executor=tool_executor,
        system_prompt=AGRONOMIST_SYSTEM_PROMPT,
    )
    app.state.agronomist_service = AgronomistService(gemini_service=gemini_service)
