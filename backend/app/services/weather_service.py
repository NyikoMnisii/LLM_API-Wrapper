...
class WeatherService:
    ...
    async def get_agricultural_forecast(self, location: str, days: int = 3) -> WeatherForecast:
        ...
        return self._forecast_from_daily(daily, best_match.display_name)

    async def get_agricultural_forecast_by_coordinates(
        self, latitude: float, longitude: float, days: int = 3, location_label: Optional[str] = None
    ) -> WeatherForecast:
        """Coordinates are unambiguous by construction, so this path skips geocoding
        and disambiguation entirely — used for the home-screen "current location"
        forecast and as the weather tool's default when the user doesn't name a
        place (see docs/ARCHITECTURE.md §4a)."""
        days = clamp_forecast_days(days)
        daily = await self._client.forecast(latitude, longitude, days)
        return self._forecast_from_daily(daily, location_label)

    def _forecast_from_daily(self, daily: dict, resolved_location: Optional[str]) -> WeatherForecast:
        if not daily:
            return WeatherForecast(
