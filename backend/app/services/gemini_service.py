...
class GeminiService:
    ...
    async def generate_structured_reply(
        self, message: str, history: list[ChatMessage], tool_executor: ToolExecutor
    ) -> AgronomistResponse:
        contents = _history_to_contents(history)
        contents.append(types.Content(role="user", parts=[types.Part.from_text(text=message)]))

        tools = [types.Tool(function_declarations=[_to_function_declaration(s) for s in tool_executor.specs])]
        ...
        while response.function_calls and hops < self._max_tool_hops:
            ...
            for call in response.function_calls:
                ...
                result = await tool_executor.execute(call.name, call.args or {})
